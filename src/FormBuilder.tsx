...
export function FormBuilder() {
  ...
  const [inheritedFromValue, setInheritedFromValue] = useState(""); // new state
  ...
  // Helper function to get the effective source of belief
  const getEffectiveSourceOfBelief = () => {
    if (sourceOfBelief === "inherited from" && inheritedFromValue.trim()) {
      return `inherited from ${inheritedFromValue.trim()}`;
    }
    return sourceOfBelief;
  };
  ...
  const exportToPDF = async () => {
    ...
    try {
      await generatePDF(
        ...
        details,
        sessionType,
        getEffectiveSourceOfBelief(),
        sectionsWithContent,
        connectedEmotionsWithContent
      );
  ...
  const handleLoadForm = (storageKey: string) => {
    loadJSON(
      ...
      setInheritedFromValue // Add this line
    );
    ...
    reader.onload = (e) => {
      try {
        ...
        if (sessionData && sessionData.title && (sessionData.sections || sessionData.connectedEmotionsSections)) {
          ...
          setInheritedFromValue(sessionData.inheritedFromValue || "");
            ...
            <div className="field">
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                ...
                >
                  ...
                  <option value="inherited from">inherited from</option>
                  <option value="from self">from self</option>
                </select>
                {sourceOfBelief === "inherited from" && (
                  <input
                    type="text"
                    value={inheritedFromValue}
                    onChange={(e) => setInheritedFromValue(e.target.value)}
                    placeholder="type source (e.g. mother, father)..."
                    className="input"
                    style={{ flex: 1 }}
                  />
                )}
            ...
            <button
              onClick={() => {
                const sessionData = {
                  ...
                  details,
                  sessionType,
                  sourceOfBelief: getEffectiveSourceOfBelief(), // Use processed value
                  sections,
                  connectedEmotionsSections
                };
            ...
            <button
              onClick={() => {
                const { subject: emailSubject, body: emailBody } = generateEmailContent(
                  ...
                  details,
                  sessionType,
                  getEffectiveSourceOfBelief(), // Use processed value
                  sections,
                  connectedEmotionsSections
                );
